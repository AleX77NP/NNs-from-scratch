...
def update_parameters(params, gradients, alfa, beta, itc):
    w1 = params['W1']
    b1 = params['B1']
    w2 = params['W2']
    b2 = params['B2']
    ...
    sdw1_c = sdw1 / (1 - beta ** itc)  # bias correction
    sdb1_c = sdb1 / (1 - beta ** itc)
    sdw2_c = sdw2 / (1 - beta ** itc)
    sdb2_c = sdb2 / (1 - beta ** itc)

    epsilon = 0.00000001

    w1 = w1 - alfa * (dw1 / (np.sqrt(sdw1_c) + epsilon))
    b1 = b1 - alfa * (db1 / (np.sqrt(sdb1_c) + epsilon))
    w2 = w2 - alfa * (dw2 / (np.sqrt(sdw2_c) + epsilon))
    b2 = b2 - alfa * (db2 / (np.sqrt(sdb2_c) + epsilon))
...
def model(x, y, n_h, alfa, reg_lambda, iterations):
    ...
    for i in range(1, iterations):
        ...
        params = update_parameters(params, gradients, alfa, beta=0.999, itc=i)
